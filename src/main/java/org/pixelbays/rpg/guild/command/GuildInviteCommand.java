...
public class GuildInviteCommand extends AbstractPlayerCommand {
    ...
    protected void execute(@Nonnull CommandContext ctx,
        ...
        if (!playerArg.provided(ctx)) {
            player.sendMessage(Message.translation("server.rpg.guild.usage.invite"));
            return;
        }
        ...
        if (result.isSuccess()) {
            targetRef.sendMessage(Message.translation("server.rpg.guild.notify.invitedBy").param("player", playerRef.getUsername()));
        }
    }
}
