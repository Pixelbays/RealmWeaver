...
public final class ClassCommandUtil {
    ...
    public static Message managerResultMessage(String message) {
        ...
        if (normalized.startsWith("Unknown class: ")) {
            return Message.translation("server.rpg.class.error.unknownClass")
        ...
        if (normalized.startsWith("Class ") && normalized.endsWith(" is not available")) {
            return Message.translation("server.rpg.class.error.notAvailable")
        ...
        if (normalized.startsWith("Already learned class ")) {
            return Message.translation("server.rpg.class.error.alreadyLearned")
        ...
        if (normalized.startsWith("Cannot learn ") && normalized.endsWith(" is learned")) {
            ...
            if (whileIndex > 0) {
                ...
                return Message.translation("server.rpg.class.error.exclusiveLearn")
        ...
        if (normalized.startsWith("Learned ")) {
            return Message.translation("server.rpg.class.success.learned")
                    .param("class", normalized.substring("Learned ".length()));
        }

        if (normalized.startsWith("Class ") && normalized.endsWith(" is not learned")) {
            return Message.translation("server.rpg.class.error.notLearnedById")
        ...
        if (normalized.startsWith("Unlearned ")) {
            return Message.translation("server.rpg.class.success.unlearned")
                    .param("class", normalized.substring("Unlearned ".length()));
        }

        if ("Must learn class before activating".equals(normalized)) {
            return Message.translation("server.rpg.class.error.mustLearnBeforeActivating");
        }

        if ("Cannot switch to this class".equals(normalized)) {
            return Message.translation("server.rpg.class.error.cannotSwitch");
        }

        if ("Cannot switch to this class while in combat".equals(normalized)) {
            return Message.translation("server.rpg.class.error.cannotSwitchCombat");
        }

        if (normalized.startsWith("Activated ")) {
            return Message.translation("server.rpg.class.success.activated")
                    .param("class", normalized.substring("Activated ".length()));
        }

        if ("Maximum combat classes reached".equals(normalized)) {
            return Message.translation("server.rpg.class.error.maxCombat");
        }

        if ("Maximum profession classes reached".equals(normalized)) {
            return Message.translation("server.rpg.class.error.maxProfession");
        }

        if (normalized.startsWith("You must select a race before learning ")) {
            return Message.translation("server.rpg.class.error.raceRequired")
        ...
        if (normalized.startsWith("Your race cannot learn ")) {
            return Message.translation("server.rpg.class.error.raceCannotLearn")
                    .param("class", normalized.substring("Your race cannot learn ".length()));
        }

        if (normalized.startsWith("Requires unknown class ")) {
            return Message.translation("server.rpg.class.error.requiresUnknownClass")
        ...
        if (normalized.startsWith("Requires ") && normalized.endsWith(" class")) {
            return Message.translation("server.rpg.class.error.requiresClass")
        ...
        if (normalized.startsWith("Requires ") && normalized.contains(" level ")) {
            ...
            if (levelIndex > "Requires ".length()) {
                ...
                return Message.translation("server.rpg.class.error.requiresLevel")
                        .param("class", className)
                        .param("level", level);
            }
        }

        return Message.translation("server.rpg.common.unmappedMessage").param("text", normalized);
    }
}