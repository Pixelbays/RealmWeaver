...
public class PlayerNameplateUpdateSystem extends DelayedEntitySystem<EntityStore> {
    ...
    public void tick(
        ...
        if (config != null && config.getNameplateSettings().isEnabled()) {
            text = resolveCustomNameplate(config.getNameplateSettings(), store, ref, playerRef, text);
        }
    ...
    @Nonnull
    private String resolveCustomNameplate(
        ...
        return PlayerNameplateFormatter.format(settings, content);
    }
