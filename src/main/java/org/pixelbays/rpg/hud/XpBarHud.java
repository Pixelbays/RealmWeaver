...
public class XpBarHud extends CustomUIHud {
    ...
    public void updateDiceRoll(@Nullable DiceRollViewData data) {
        if (data == null || data.diceValues.length == 0) {
            if (lastDiceSignature.isEmpty()) {
                return;
            }
        ...
        String signature = buildDiceSignature(data);
        if (safeEquals(signature, lastDiceSignature)) {
            return;
        }
        ...
        cmd.clear(SELECTOR_DICE_SMALL_HOST);
        cmd.clear(SELECTOR_DICE_LARGE_HOST);
        cmd.appendInline(data.mode == DiceOverlayMode.LARGE ? SELECTOR_DICE_LARGE_HOST : SELECTOR_DICE_SMALL_HOST,
    ...
    @Nonnull
    private static String buildDiceSignature(@Nonnull DiceRollViewData data) {
    ...
    private static String buildDiceOverlayMarkup(@Nonnull DiceRollViewData data) {
        ...
        ui.append("    Text: \"").append(escapeUiString(data.notation)).append("\";\n");
        ...
        ui.append("    Text: \"").append(escapeUiString(data.summary)).append("\";\n");
