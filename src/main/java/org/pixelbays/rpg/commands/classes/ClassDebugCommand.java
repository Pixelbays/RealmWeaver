...
public class ClassDebugCommand extends AbstractPlayerCommand {
    ...
    protected void execute(@Nonnull CommandContext ctx,
        ...
        LevelSystemConfig levelConfig = systemId != null && !systemId.isEmpty() ? levelSystem.getConfig(systemId) : null;
        ...
        lines.add("StartingClass: " + classDef.isStartingClass() + ", UsesCharacterLevel: " + classDef.usesCharacterLevel());
        lines.add("LevelSystemId: " + classDef.getLevelSystemId());
        lines.add("Learned: " + learned + ", Active: " + active + ", CurrentLevel: " + currentLevel + ", ExpToNext: " + expToNext
        ...
        if (!classDef.getResourceStats().isEmpty()) {
            //lines.add("ResourceStats: " + String.join(", ", classDef.getResourceStats().keySet()));
        }
        ...
        if (baseStats != null && !baseStats.isEmpty()) {
            ...
            baseStats.getMultiplicativeModifiers().forEach((stat, value) -> lines.add("  +" + (value * 100) + "% " + stat));
        }
        ...
        if (perLevelStats != null && !perLevelStats.isEmpty()) {
            ...
            perLevelStats.getMultiplicativeModifiers().forEach((stat, value) -> lines.add("  +" + (value * 100) + "% " + stat));
        }
        ...
        if (!classDef.getLevelMilestones().isEmpty()) {
            ...
            for (ClassDefinition.LevelMilestone milestone : classDef.getLevelMilestones()) {
                lines.add("  - level " + milestone.getLevel() + ", skillPoints " + milestone.getSkillPoints() + ", items " + milestone.getItemRewards().size()
        ...
        if (levelConfig == null) {
            ...
            lines.add("LevelSystemStartingLevel: " + levelConfig.getStartingLevel() + ", MaxLevel: " + levelConfig.getMaxLevel());
            lines.add("LevelSystemIconId: " + levelConfig.getIconId());
            lines.add("LevelSystemInheritsFrom: " + levelConfig.getInheritsFrom());
            lines.add("LevelSystemExpCurveType: " + levelConfig.getExpCurveType() + ", ExpCurveRef: " + levelConfig.getExpCurveRef());

            LevelSystemConfig.ExpCurveConfig expCurve = levelConfig.getExpCurve();
            if (expCurve != null) {
            ...
            if (defaultRewards != null && hasAnyRewards(defaultRewards)) {
                lines.add("DefaultRewards: statPoints=" + defaultRewards.getStatPoints()
                    ...
                    + (defaultRewards.getInteractionChain() != null ? ", interactionChain=" + defaultRewards.getInteractionChain() : ""));
            ...
            if (statGrowth != null) {
                ...
                int percentCount = statGrowth.getPercentageGrowth() == null ? 0 : statGrowth.getPercentageGrowth().size();
                int milestoneCount = statGrowth.getMilestoneGrowth() == null ? 0 : statGrowth.getMilestoneGrowth().size();

                if (flatCount > 0 || percentCount > 0 || milestoneCount > 0) {
                    lines.add("StatGrowth: flat=" + flatCount + ", percent=" + percentCount + ", milestones=" + milestoneCount);
                }
            }
        }
