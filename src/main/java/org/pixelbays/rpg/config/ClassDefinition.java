...
public class ClassDefinition implements JsonAssetWithMap<String, DefaultAssetMap<String, ClassDefinition>> {
    ...
    private static final class LevelSystemRefCodec implements ValidatableCodec<String> {
        @Nonnull
        @SuppressWarnings("null")
        private AssetCodec<String, LevelSystemConfig> getAssetCodec() {
            return AssetRegistry.getAssetStore(LevelSystemConfig.class).getCodec();
        }
        ...
        public Schema toSchema(@Nonnull SchemaContext context) {
            StringSchema schema = new StringSchema();
            schema.setTitle("Level System");
            schema.setHytaleAssetRef(LevelSystemConfig.class.getSimpleName());
            return schema;
        }
        ...
        public void validate(String value, ExtraInfo extraInfo) {
            ValidationResults results = extraInfo.getValidationResults();
            if (results != null) {
                LevelSystemConfig def = AssetRegistry.getAssetStore(LevelSystemConfig.class).getAssetMap().getAsset(value);
                if (def == null) {
                    results.fail("Unknown level system id: " + value);
                }
            }
        }
